...
\chapter{理論:3Dモデルの製作}
	...
	\section{EinScan HX}
		...
		\subsection{製品仕様}
			\cite{ref:4}から参照したEinScanの製品仕様を図\refeq{fig:EinScan}に示す.
			\begin{table}[H]
			\begin{center}
...
\clearpage
		...
		ジェスチャ判定時には画面右下のバーが白色から緑色に変化するため,筋変位センサの測定値は
		バーの色が切り替わるタイミングに統一して測定した.
...
\clearpage
		...
		この結果を用いて,同sample数における被験者ごとの数値の分散が少ないsample数が最適としたため,
