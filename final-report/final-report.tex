...
\chapter{3Dモデルの製作}
	...
	\section{Unity}
		...
		\subsection{アニメーション}
			...
			図\refeq{fig:animater}ではシーンの実行直後に緑色ブロックのEntryからオレンジブロックのGrapSphereに遷移し,
			アニメーションが実行され,赤色ブロックのEndに遷移する.加えてアニメーター機能では,遷移条件を付加することも可能である.
			...
			図\refeq{fig:ObjectHold}にオブジェクト保持表現のアニメーションを示す.
			\begin{figure}[H]
			\centering
...
\chapter{研究結果}
	...
	\section{オブジェクト保持}
		...
		アニメーター構成は図\refeq{fig:Handanimater}のようになっており,各アニメーションの遷移フラグとしてgrap\_sphere1,\\
		grap\_cylinder1,grap\_cube1を用意した.何もしていない状態でnutralというアニメーションクリップをループ再生し続け,衝突判定とオブジェクト保持のフラグが両方立った場合に
		\begin{figure}[H]
		\centering
...
\chapter{考察}
	FirstVRを入力インターフェースとすることで実際の筋電義手シミュレータよりもより安価で
	PC版のシミュレータよりも没入感が高くすることができた．しかし,
